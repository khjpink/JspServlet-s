...
<html>
	...
	<body>
		<%
			...
			//회원가입 정보 가져오기 
			//이건 전체회원정보 조회하는 매퍼랑 메소드 만들어야함. 거기에 세션 넣기
			//Member join = (Member)session.getAttribute("loginSession");
...
<%-- 			<% if(loginMember == null){%>
			...
			<% }else{%>
				...
				<a href="update.jsp">개인정보수정</a> 어떤 정보를 수정할지 받아서 보내야하기 때문에 서블릿이 아닌jsp임
			<%} %> --%>
				
				<!-- jstl은 if else가 없어서 wher, otherwise를 쓴 것 -->
				<c:choose>
					<c:when test="${empty loginMember}">
						<a href="#menu">로그인</a>
					</c:when>
					<c:otherwise> <!-- equal 이퀄 = , 로그인멤버에서 이메일 꺼내서 = admin문자열 이라면 -->
 				...
 				<section class="split">
					<section>
						<div class="contact-method">
							...
							<h3>Email</h3>
							<!-- jstl로 로그인 한 사용자의 이메일을 출력하시오
							EL식으로 쓰는 속석명에다가 Member객체의 필드명으로 꺼내옴
							Member loginMember = (Member)session.getAttribute("loginMember");
							표현식으로는 loginMember.getEmail() (왼쪽 변수명.게터로 가져옴)
							EL식으로는 loginMember.email(오른 세션에 저장된 이름.Member객체필드명) -->
							
							<c:choose>	
								<c:when test="${empty loginMember}">
									<a href="#">로그인 한 사람의 이메일이 출력</a>	
								</c:when>
								<c:otherwise>
					...
					<section>
						<div class="contact-method">
							...
							<c:choose>
								...
								<c:otherwise>
									<span>${sessionScope.loginMember.address}</span>
